...
export default function CompoundInterestCalculator() {
  ...
  const calculate = () => {
    ...
    const fv = p * Math.pow(1 + r / n, n * t) +
      (rm === 0 ? pmt * months : pmt * ((Math.pow(1 + rm, months) - 1) / rm));

    const totalDeposits = p + pmt * months;
    const interest      = fv - totalDeposits;
  ...
  const clear = () => {
    setPrincipal(5000); setRate(8); setCompound("monthly");
    setYears(10); setMonthlyAdd(100); setResult(null);
  };

  return (
    <div className="mc-page">
      <div className="mc-breadcrumb">
        <Link href="/">home</Link> / <Link href="/">financial</Link> / compound interest calculator
      </div>

      <div className="mc-layout">
        <div className="mc-main">
          <h1 className="mc-title">Compound Interest Calculator</h1>
              ...
              <div className="mc-results">
                ...
                <table className="mc-table">
                  ...
                  <tbody>
                    ...
                    <tr><td>Total Interest Earned</td><td>{fmt(result.interest)}</td></tr>
                ...
                <table className="mc-summary">
                  <tbody>
                    <tr><td>Principal</td><td>{fmt(principal)}</td></tr>
                    <tr><td>Monthly Addition</td><td>{fmt(monthlyAdd)}</td></tr>
                    <tr><td>Annual Rate</td><td>{rate}%</td></tr>
                    <tr><td>Compound</td><td style={{ textTransform: "capitalize" }}>{compound}</td></tr>
                    <tr><td>Time Period</td><td>{years} years</td></tr>
                    <tr><td>Total Deposits</td><td>{fmt(result.totalDeposits)}</td></tr>
                    <tr><td>Interest Earned</td><td>{fmt(result.interest)}</td></tr>
