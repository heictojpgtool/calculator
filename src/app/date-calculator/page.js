...
export default function GradeCalculator() {
  ...
  useEffect(() => { calculate(); }, []);
  const clear = () => { setAssignments([{ name: "", score: 0, weight: 0 }]); setResult(null); };

  return (
    <div className="mc-page">
      <div className="mc-breadcrumb"><Link href="/">home</Link> / <Link href="/">other</Link> / grade calculator</div>
      <div className="mc-layout">
        <div className="mc-main">
          <h1 className="mc-title">Grade Calculator</h1>
          <div className="mc-infobar"><span className="mc-arrow">▼</span>Enter assignments, scores and weights then click Calculate</div>
          <div className="mc-body" style={{ flexDirection: "column" }}>
            <table className="mc-table" style={{ marginBottom: "12px" }}>
              <thead>
                <tr><th>Assignment</th><th>Score (%)</th><th>Weight (%)</th></tr>
              </thead>
              <tbody>
                {assignments.map((a, i) => (
                  <tr key={i}>
                    <td><input value={a.name} onChange={(e) => update(i, "name", e.target.value)} style={{ width: "130px", border: "1px solid #aaa", padding: "3px 5px" }} /></td>
                    <td><input type="number" value={a.score} onChange={(e) => update(i, "score", e.target.value)} style={{ width: "70px", border: "1px solid #aaa", padding: "3px" }} /></td>
                    <td><input type="number" value={a.weight} onChange={(e) => update(i, "weight", e.target.value)} style={{ width: "70px", border: "1px solid #aaa", padding: "3px" }} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div style={{ marginBottom: "12px" }}>
              <button className="mc-clear-btn" onClick={() => setAssignments([...assignments, { name: "", score: 0, weight: 0 }])}>+ Add Row</button>
            </div>
            ...
            {result && (
              <div className="mc-results" style={{ marginTop: "16px" }}>
                <div className="mc-monthly-pay"><span>Final Grade:</span><strong>{result.grade}% ({result.letter})</strong></div>
                <table className="mc-summary">
                  <tbody>
                    <tr><td>Weighted Grade</td><td>{result.grade}%</td></tr>
                    <tr><td>Letter Grade</td><td>{result.letter}</td></tr>
                    <tr><td>Total Weight</td><td>{result.totalWeight}%</td></tr>
                  </tbody>
                </table>
              </div>
            )}
